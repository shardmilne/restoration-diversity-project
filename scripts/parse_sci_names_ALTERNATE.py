...
#### This one keeps the quotation marks
...
def parse_sci_name(text):
	...
	while i < len(words):
		...
		if i == 0: # The first word is ALWAYS the genus
			sci_name += words[i]
		elif words[i] == "ex" or words[i] == '' or words[i] == "non" or words[i] == "auct." or words[i] == "bis": # These are the lowercase phrases we want to exclude
			pass 
...
def get_sci_names(database_file, new_file):
	...
	while i < len(db_lines):
		...
		# line = db_lines[i].split(",") # separate using commas - doesn't work for national DB
		line = db_lines[i].split('"')[1::2] # separate by capturing things BETWEEN quotation marks
		if len(line) > 1:
			sci_name = parse_sci_name(line[2])
			# if sci_name[-1] != '\"':
			# 	# Check if it has the closing quotation mark, and if not, add it
			# 	sci_name = sci_name + '\"'
